...
from config import PATCH_SIZE_X, PATCH_SIZE_Y, STRIDE_X, STRIDE_Y, PREPROCESSING_PARAMS, STORAGE, MapType
...
class DatasetInstance:
    ...
    def set_up_data(self):
        ...
        if self.is_in_validation():
            print(f"Create data for testing/validation")
            ...
            def filename(name):
                ...
                path = os.path.join(
                    STORAGE, 'val' if self.is_in_validation() else 'test', filename)
                return path
            ...
            for _coords in tqdm(coords):
                def filename(name):
                    ...
                    path = os.path.join(
                        STORAGE, 'train' if self.use_real else 'train_unlabelled', filename)
                    return path

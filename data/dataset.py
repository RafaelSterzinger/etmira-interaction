...
from config import CHANNELS, SAMPLE_SIZE, get_slice
...
class EtMirADatasetValidation(Dataset):
    def __init__(self, data_path=None, transform_input=None, transform_gt=None, patch_size=512, return_coords=False, id=None):
        super().__init__()
        self.image_paths = sorted(
            glob.glob(os.path.join(data_path, "*_inputs.npy")))
        if id:
            self.image_paths = list(
                filter(lambda x: id in x, self.image_paths))
        self.gt_paths = list(
            map(lambda x: x.replace('inputs', 'gt'), self.image_paths))

    ...
    def step(self, epoch=None):
        if epoch == None:
            if self.T_cur + 1 == self.T_i:
                ...
                self.base_lrs = [
                    base_lr * self.decay for base_lr in self.base_lrs]
        else:
            if epoch < 0:
                raise ValueError(
            ...
            if epoch >= self.T_0:
                if self.T_mult == 1:
                    ...
                    n = int(
                        np.log((epoch / self.T_0 * (self.T_mult - 1) + 1), self.T_mult))
            else:
                n = 0

            self.base_lrs = [initial_lrs * (self.decay**n)
                             for initial_lrs in self.initial_lrs]

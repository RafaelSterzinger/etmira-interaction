...
def visualize_human_input(prediction_mask, human_input):
    ...
    rgb_image[prediction_mask] = [0, 0, 0]  # Black

    # Pixels in prediction but not in ground truth (Green)
    rgb_image[human_input == 1] = [0, 255, 0]  # Green

    # Pixels in ground truth but not in prediction (Red)
    rgb_image[human_input == -1] = [255, 0, 0]  # Red
...
def log_image(x, target, preds, human_input, y_init):
    x = stack_batch(x)
    ...
    if human_input is not None:
        bool_mask_init = (y_init >= 0.5).cpu().numpy().astype(bool)
        temp = torch.tensor(np.array([visualize_human_input(bool_mask_init[i], human_input[i])
                                      for i in range(human_input.shape[0])]), dtype=float)
        human_input = stack_batch(temp)
        images += [human_input]
